...
| [2-4 五种错误的上下文管理](2-4-bad-context-patterns/README.zh-CN.md) | 破坏缓存 vs 破坏能力 | 两类破坏代价完全不同；**量缓存必须交错测**，顺序跑会把变量绑死 | ✅ |
...
| 2-4 五种错误的上下文管理 | **2-3** | kv-cache | 原书 5 种错误模式（含**动态用户配置**）+ 正确基线，六种全做了；历史程序化写死；原书用 Kimi 读 `cached_tokens`，本地只能读预填充耗时，所以额外加了 `cache` 交错测量模式 |
