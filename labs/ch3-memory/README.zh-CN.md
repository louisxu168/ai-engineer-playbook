...
## 和原书一一对应：13 个项目，做了 4 个

原书第 3 章有 **13 个配套项目**。**本章目前做了 4 个**，
...
| **3-4** | `dense-embedding` | [3-4-dense-embedding](3-4-dense-embedding/README.zh-CN.md) | ✅ |
...
所以 `retrieval-pipeline`(3-6)、`contextual-retrieval`(3-11) 这些**技术上没有障碍**。
它们没做的真实原因就一句话：**我还没做。**

`dense-embedding`(3-4) 已经按这条路做出来了 —— 它用的就是上面那个
`nomic-embed-text`，零 API key。**所以那个「技术障碍」是彻底不存在的，
不是「暂时绕过了」。**
...
| [3-4 稠密向量检索](3-4-dense-embedding/README.zh-CN.md) | 「上 embedding 就好了」是真的吗 | 稠密不是稀疏的升级版，是**换了一种漏法**；最危险那条六种配置全漏 |
...
| 3-4 | 原书用 ANNOY / HNSW 两个库；这里两个索引都**从零写**（只用 numpy，`annoy` 的 wheel 在本机是坏的，详见 SOLUTION）。另外加了一半原书没有的内容：**和 3-5 用同一份语料同一个问题做稀疏/稠密对照** |
...
> **3-5 失效的那个地方，正是 embedding 存在的理由** —— 于是有了 **3-4**。
>
> 但 3-4 的实测结论不是「embedding 解决了」：
> 那条「误食会送医院」的记忆，**两种方法 × 两个模型 × 两种语言，六种配置全漏**。
> 稠密检索把「不吃辣」从 #4 提到了 #1（真本事），
> 同时把花生过敏留在 #21/36（救不回来）。
>
> **先做 3-5，再做 3-4。** 顺序反了就浪费了 3-5 结尾那个悬念。
