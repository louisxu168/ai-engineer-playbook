...
*Hands-on labs for understanding how AI agents actually work. No API key required —
runs on your existing Claude Code / Codex subscription.*
...
这个 Playbook 的做法是：**每个实验都是一个约 150 行、零框架、能一口气读完的独立脚本**，
...
| 02 | 工具设计 | schema 怎么写、描述写烂了会怎样、报错怎么回传给模型 | 📋 计划中 |
| 03 | 跨会话记忆 | 文件式 memory：什么该记、什么不该记 | 📋 计划中 |
| 04 | 上下文压缩 | 对话撑爆窗口时，压缩和裁剪的区别 | 📋 计划中 |
| 05 | 检索增强 | 为什么朴素 RAG 打不过 agentic search | 📋 计划中 |
| 06 | 规划与子代理 | 什么时候该拆任务，什么时候拆了反而更慢 | 📋 计划中 |
| 07 | 怎么评测 agent | LLM-as-judge，以及它什么时候不靠谱 | 📋 计划中 |
| 08 | 从经验中学习 | 让 agent 把踩过的坑写下来，下次少踩 | 📋 计划中 |
...
├── .claude/commands/
│   └── lab.md             /lab 01 这个 slash command
└── labs/
    └── 01-context/        每个实验一个独立文件夹
        ├── README.md      实验说明：概念 + 步骤 + 练习
        ├── agent.py       主体，约 150 行，一口气读得完
        ├── llm.py         后端适配（claude / codex / api）
        ├── AGENTS.md      这个实验专属的助教指令
        ├── SOLUTION.md    参考答案 —— 自己试过之后再看
        └── requirements.txt
```
